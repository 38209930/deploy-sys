...
- 尚未完成：各代码会话的完整评审、镜像运行与 `release` SHA 冻结；实际启用的外部调用/SDK/白名单清单；NAT 生产网络变更授权和付费采购；镜像仓库、镜像 digest、生产配置 Secret、Worker 及 API 内消费者的交接核对、业务验收；经销商旧门店表到独立新宿主的数据迁移。
- 出口复核：目标 VPC 的公网 NAT 网关为 0；七个 vSwitch 共用一张无默认路由的系统表。首次创建 NAT 会自动添加默认路由，影响全部关联 vSwitch 的路径。ACS 北京 k 的 `172.31.224.0/20` 内还有范围外网站 ECS，故不得直接对现有两个 ACS 网段建立整段 SNAT。隔离拓扑、CIDR 与逐步回滚已形成[生产网络变更单](egress-nat-change-order.md)，待授权实施。

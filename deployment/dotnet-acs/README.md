...
具体资源与域名见 [资源清单](inventory.md)，逐项目阻断项见 [五项目发布记录](release-records.md)，公共公网出口见 [实施前复核与落地清单](egress-proxy.md)，操作顺序见 [发布手册](runbook.md)，故障与费用见 [运维备忘录](operations.md)。这些文件不包含凭据、配置值和客户数据。
...
- 出口复核：目标 VPC 的公网 NAT 网关只读查询为 0；独立代理 ECS/EIP 尚无可核对的资源 ID 或验收记录。当前两个 ACS vSwitch 与 Java Pod 共用，不直接在其上建立覆盖所有 Pod 的 SNAT。单机代理的恢复时间和 SDK 兼容性尚待实测；这两项是首个 .NET 项目切换前的门槛，详见[出口实施清单](egress-proxy.md)。

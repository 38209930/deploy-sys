...
具体资源与域名见 [资源清单](inventory.md)，逐项目外呼见 [公共出口核对表](external-dependencies.md)，.NET 阻断项见 [五项目发布记录](release-records.md)，公共公网出口见 [NAT 执行手册](egress-nat.md)，操作顺序见 [发布手册](runbook.md)，故障与费用见 [运维备忘录](operations.md)。这些文件不包含凭据、配置值和客户数据。
...
- 尚未完成：各代码会话的完整评审、镜像运行与 `release` SHA 冻结；实际启用的外部调用/SDK/白名单清单；NAT 路由隔离设计和付费采购；镜像仓库、镜像 digest、生产配置 Secret、Worker 及 API 内消费者的交接核对、业务验收；经销商旧门店表到独立新宿主的数据迁移。
- 出口复核：目标 VPC 的公网 NAT 网关为 0；七个 vSwitch 共用一张无默认路由的系统表。首次创建 NAT 会自动添加默认路由，影响全部关联 vSwitch 的路径。ACS 北京 k 的 `172.31.224.0/20` 内还有范围外网站 ECS，故不得直接对现有两个 ACS 网段建立整段 SNAT。先完成[路由及来源隔离修订](egress-nat.md)。
- 因上述缺口，本轮**尚未采购 NAT/EIP，未创建 .NET Namespace/镜像仓库/Ingress，未启动任何新 Pod，未停旧服务，未修改 DNS 或既有 Java 资源**。

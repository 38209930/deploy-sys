...
4. 冻结数据库结构与配置版本。证明旧新 API 并行期兼容、连接池总数可承受、无启动自动迁移和不受控 HostedService。经销商的单体 `agent-query-api` 沿用原生产库，须验证 ACS 到 MySQL、Redis 的连通、原有 OSS 图片路径和前后台接口契约。
...
- 经销商：回滚前核对同一生产库中由新版本产生的写入能否被旧版识别，确认后再恢复旧入口；图片继续使用原 OSS，不以本地 Pod 目录保存。

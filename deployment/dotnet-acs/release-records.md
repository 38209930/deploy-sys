...
AI 自习室 Back 初次仅验证了健康接口，后台前端实际调用时因生产配置缺少 `Security:CorsAllowedOrigins` 被浏览器拦截。已在 Deployment 运行环境中补入 `https://ai-study-manage.svision100.com`、`https://ai-study-biz.svision100.com`、`https://ai-study-h5.svision100.com`，并追加核验用的 `https://agent-admin.svision100.com`，完成滚动更新。四来源预检均返回 204，`POST /auth/login` 返回业务层响应且带正确 CORS 头；`/health/live`、`/health/ready`、`/back/public/info` 均返回 200。镜像 digest 未变更，当前 Back Pod 1/1 Ready、重启次数为 0。进一步核对发现，线上 `ai-study-manage.svision100.com` 页面仍是旧 PC 构建，JS 将主 API 固定为 `https://rsapi.svision100.com`，商城 API 固定为 `https://rsst-front-api.svision100.com`，其登录路径为 `/login`；新 ACS BackApi 的登录路径为 `/auth/login`，两者不能仅靠 ALB Host 或 CORS 直接替换。自习室后台前端需在独立开发/发布任务中完成 API 地址和接口契约适配后，才能认定迁移完成。`agent-admin.svision100.com` 是经销商查询 PC 页面，线上 OSS 返回 200 且构建包指向 `rsqapi-bk.svision100.com`，不属于自习室后台。

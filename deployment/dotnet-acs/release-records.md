...
售后两个 Ingress 曾因短信旧通道仍启用而撤下，致使域名返回 503；已按授权将生产短信路由改为仅启用私网 SmsCore，并恢复两个 Host 规则。AI 的 SmsCore 运行配置已补齐。核验过程中 AI 专用短信凭据曾进入受控命令输出，已轮换并禁用旧凭据；不在本记录保存凭据值。健康检查只证明入口及主要依赖就绪，真实短信与业务流程仍需单独验收。经销商当前只确认 `4l-api.svision100.com` 为 ACS Ingress Host；历史记录中的 `rsqapi-ft`、`rsqapi-bk` 不作为当前已接入域名依据。七个当前 .NET API Host 的公网 HTTPS `/health/ready` 均在 23:39 核验返回 200；部分 API 根路径 `/` 返回 404，不能据此判定域名未接入。
...
- 正式 Host：当前只将 `4l-api.svision100.com` 作为已核实的 ACS ALB Host，由 `agent-query-api-alb` Ingress 接入同一个单体 API；不需要为前后台分别创建 Deployment。`rsqapi-ft.svision100.com`、`rsqapi-bk.svision100.com` 属于历史记录，未在本次当前 Ingress 读回中作为现行 Host 记录。

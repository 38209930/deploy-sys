...
| 外部依赖 | 实际启用的目的域名/端口、调用 SDK 真实代理证据、非 HTTP 处理、超时/重试/幂等、回调地址、旧新出口 IP 双白名单及核对证据：待验证；按[出口实施清单](egress-proxy.md)登记 |
| 旧服务 | systemd/容器实例及自启方式、Worker 停机命令、在途任务与锁、API 保留时间、回滚启动命令：待验证 |
| 网络 | 私网数据库/Redis/Mongo/SmsCore 连通性、出口代理允许/拒绝及公网 3128 拒绝测试、供应商侧来源 EIP、单机恢复 RTO、ALB 证书与 Host 路由、DNS TTL：待验证 |

...
deploySys 本机私有配置中已拆为两个独立入口：`M1X -> m1x-api-new -> prod` 只构建、发布和检查 API；`M1X -> m1x-worker-new -> prod` 只构建、发布、重启和检查 Worker。原有 `M1X Java API` 入口保持不变。Worker 发布采用单实例停机替换，验收失败时恢复上一版 Jar。

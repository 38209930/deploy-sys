...
- 桌面客户端支持直接编辑命令预览并保存
- 桌面客户端新增服务时可一次填写服务 ID、执行目标和执行命令
- 桌面客户端新增执行目标时可一次填写目标名称和执行命令
- 桌面客户端使用固定列表选择执行目标和动作，避免 macOS 下拉控件卡顿
- 桌面客户端执行日志采用批量刷新，并只保留最近日志，避免长输出导致界面卡顿
- 执行日志本地保存，并对疑似敏感值做脱敏
- 可查看项目配置和配置文件路径
- 可删除项目、子任务或环境下保存的命令

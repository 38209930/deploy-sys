...
[ "$worker_enabled" = "disabled" ] || { echo "ERROR: deferred Worker must remain disabled" >&2; exit 1; }
[ "$worker_active" = "inactive" ] || { echo "ERROR: deferred Worker must remain inactive" >&2; exit 1; }
...
remote_deploy() {
  ...
  if ! ssh "${SSH_OPTIONS[@]}" "$SSH_TARGET" bash -s -- \
    "$WORKER_SERVICE" "$REMOTE_JAR" "$REMOTE_CONFIG" "$REMOTE_STAGE" "$LOCAL_SHA" "$RELEASE_ID" <<'REMOTE'
set -euo pipefail
worker_service="$1"
remote_jar="$2"
remote_config="$3"
...
worker_enabled="$(systemctl is-enabled "$worker_service" 2>/dev/null || true)"
worker_active="$(systemctl is-active "$worker_service" 2>/dev/null || true)"
[ "$worker_enabled" = "disabled" ] || { echo "ERROR: Worker must be disabled before deferred deployment" >&2; exit 1; }
[ "$worker_active" = "inactive" ] || { echo "ERROR: Worker must be inactive before deferred deployment" >&2; exit 1; }
...
[ "$running_sha" = "$expected_sha" ] || {
  ...
  echo "ERROR: installed Worker jar SHA-256 mismatch; previous jar restored" >&2
  exit 1
}
